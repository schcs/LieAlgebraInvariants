...
def _is_element_of_subalgebra(gens, denoms, p):
    '''
    INPUT:

    - a list of polynomials `gens`
    - a list of denominators
    - a polynomial `p`

    OUTPUT:

    - true/false depending on whether p lies in the algebra generated by gens
      and the inverses of the polynomials in denoms
    - in case of true, returns a polynomial r such that r( gens ) == f*p where
      f is a product of polynomials in genoms
    ...
    lm_denoms = [x.lm() for x in denoms]
    gensP = p.parent().gens()
    nr_gensP = len(gensP)
    nr_gens = len(gens)
    ...
    genst = dict(zip(gens, Pt.gens()[0:nr_gens]))
    genst.update(dict(zip(denoms, Pt.gens()[nr_gens:nr_gens+nr_denoms])))
    ...
    while p != 0:
        ...
        lm_p = p.lm()
        for g in gensP:
            lm_p_degree_g = P(lm_p.numerator()).degree(g) - P(lm_p.denominator()).degree(g)
            if lm_p_degree_g > 0:
        ...
        if lm_p != 1:
            ...
            for i in range(len(denoms))[::-1]:
                den = lm_denoms[i]
                while lm_p_denom % den == 0:
                    lm_p_denom //= den
                    p *= denoms[i]
                    den_mon *= Pt.gens()[nr_gens+i]
        
        coeff = QQ(-p.coefficient(p.lm())*reduction_pol.coefficient(reduction_pol.lm()))
        tpol -= coeff*tmon/den_mon
        p += coeff*reduction_pol
    print("tpol is ", tpol)
    assert tpol.subs(dict(zip(Pt.gens(), list(gens)+denoms))) == p_orig
    print("test passed")
