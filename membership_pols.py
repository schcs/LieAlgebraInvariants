...
def _is_element_of_subalgebra(gens, denoms, p, Pt=False):
    '''
    INPUT:
    ...
    while p != 0:
        ...
        if lm_p != 1:
            ...
            # leading terms of the denominators in reverse order
            for i in range(len(denoms)):
                den = lm_denoms[i]
                # if lm_p_denom is divisible by den, then divide
                if den % lm_p_denom == 0:
        ...
        #    assert p.lm() < lm_old
        #except: 
        #    print("new lm is not smaller")
            
    # check that the result is correct
    nr_zeros = Pt.ngens()-len(gens)-len(denoms)
    assert tpol.subs(dict(zip(Pt.gens(), list(gens)+[0]*nr_zeros+denoms))) == p_orig
